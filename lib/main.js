...
export default {
  ...
  provideAutoComplete: function() {
    ...
    const Score = require('sb-string_score')
    const Regex = {
      isClass: / class$/,
      label: /(\w+)/,
      prefix: /((:\w[\w:-]+)|(\$\w+)|::(\w+)|(\w+))$/
    ...
    const Provider = {
      ...
      parseSuggestion: (givenSuggestion, prefix) => {
        const spaceIndex = givenSuggestion.indexOf(' ')
        const suggestion = [givenSuggestion.slice(0, spaceIndex), givenSuggestion.slice(spaceIndex + 1) || '_']
        ...
        return {
          ...
          score: Score(suggestion[0], prefix)
        }
      },
      getSuggestions: ({editor, bufferPosition}) => {
        ...
        return this.Hack.exec(atom.config.get('Atom-Hack.hackExecutablePath'), ['--auto-complete'], {
          ...
          const toReturn = []
          let prefix = (Regex.prefix.exec(editor.getTextInRange([[bufferPosition.row, 0], bufferPosition])) || [])
          prefix = prefix[2] || prefix[3] || prefix[4] || prefix[5] || ''
          contents.substr(0, contents.length - 1).split("\n").forEach(entry => {
            if (entry.length) {
              toReturn.push(Provider.parseSuggestion(entry, prefix))
            }
          })
